...
class Utils():
    ...
    def _get_peer_property(fl_ctx: FLContext, prop):
        peer_ctx = fl_ctx.get_peer_context()
        value = peer_ctx.get_prop(prop, None)
        if value is None:
    ...
    def get_peer_job_name(fl_ctx: FLContext):
        return Utils._get_peer_property(fl_ctx, Constants.JOB_NAME)
          
    def get_peer_job_start_timestamp(fl_ctx: FLContext):
        return Utils._get_peer_property(fl_ctx, Constants.JOB_START_TIMESTAMP)