...
class SimpleTrainer(Executor):
    ...
    def _initWandB(self, fl_ctx: FLContext, project=PROJECT_NAME):
        ...
        if wandb_id is None:
            ...
            wandb_id = TF2Utils.generate_wandb_id(
                TF2Utils.get_peer_job_name(fl_ctx),
        ...
        self.wandb = wandb.init(
            ...
            config={
                ...
                'job_name': TF2Utils.get_peer_job_name(fl_ctx),
                'job_start_timestamp': TF2Utils.get_peer_job_start_timestamp(fl_ctx),
    ...
    def _deinitWandB(self, fl_ctx: FLContext):
        if self.wandb is not None:
            self.log_info('de-initializing WandB...')
            self.wandb.finish()
            self.log_info('WandB finished')
